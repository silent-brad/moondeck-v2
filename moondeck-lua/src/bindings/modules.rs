...
fn load_lua_module<'gc>(
    ...
    name: &str,
    source: &str,
) -> Result<Value<'gc>, piccolo::Error<'gc>> {
    ...
    lua.try_enter(|ctx| {
        ...
        for name in THEME_NAMES {
            themes.set(
                ...
                create_theme_colors_table(ctx, name)?,
            )?;
        }
        ...
        theme_table.set(
            ctx,
            "current",
            ctx.intern(CURRENT_THEME.read().unwrap().as_bytes()),
        )?;
    ...
    ctx.set_global(
        ...
        Callback::from_fn(&ctx, move |ctx, _exec, mut stack| {
            ...
            if matches!(result, Value::Nil) {
                ...
                return Err(piccolo::Error::from_value(
                    ctx.intern(msg.as_bytes()).into(),
                ));
            }
