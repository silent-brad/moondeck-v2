...
fn run_loop(
    ...
    loop {
        ...
        // Update all widgets (not just the current page) so WiFi-dependent
        // widgets on other pages can pre-fetch data in the background.
        // Each widget's Lua-side update() already throttles via its own
        // fetch_interval timer, so this doesn't cause extra API calls.
